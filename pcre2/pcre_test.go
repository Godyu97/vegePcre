...
func TestMatchUngreedy(t *testing.T) {
	r := pcre2.MustCompileOpts(`Hello, (.+)\.`, pcre2.Ungreedy)
	defer r.Close()
...
func TestReplace(t *testing.T) {
	...
	newStr = r.ReplaceAllString(testStr, "$1hello")
	if newStr != "123hello Test" {
		t.Errorf(`expected "123.12345 Test", got "%s"`, newStr)
	}
...
func TestSubexpIndex(t *testing.T) {
	r := pcre2.MustCompile(`(?P<number>\d)`)
	defer r.Close()
