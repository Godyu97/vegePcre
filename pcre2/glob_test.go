...
func TestGlob(t *testing.T) {
	...
	matches, err := pcre2.Glob("pcretest")
	if err != nil {
		t.Fatal(err)
	}
	...
	matches, err = pcre2.Glob("pcretest/dir*")
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) != 2 ||
	...
	matches, err = pcre2.Glob("pcretest/file*")
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) != 2 ||
	...
	matches, err = pcre2.Glob("pcretest/**/*.txt")
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) != 1 ||
